...
config.keys = {
  ...
  -- { key = "y",          mods = "CTRL",         action = action.Multiple { action.CopyTo("ClipboardAndPrimarySelection"), action.ClearSelection } },
  { key = "c",          mods = "CMD",         action = action.Multiple { action.CopyTo("ClipboardAndPrimarySelection"), action.ClearSelection } },
...
table.insert(config.hyperlink_rules, {
-- /Users/you/project 같은 절대 경로를 매칭
    regex = [[((/Users/[^ \t\r\n:"'<>()]+)+/?)]],
	format = 'file://$1',
})
...
--[[
  ...
  {
    label = "Bastion - DEV",
    args = { "zsh", "-lc", "~/bastion dev"},
  },
  {
    label = "Bastion - QA",
    args = { "zsh", "-lc", "~/bastion qa"},
  },
...
wezterm.on("update-status", function(window, pane)
  ...
  --]]
  local cwd = pane:get_current_working_dir()
  if cwd and type(cwd) == "userdata" then
	cwd = cwd.file_path or ""
	if string.len(cwd) > config.tab_max_width then
		cwd = ".." .. string.sub(cwd, config.tab_max_width * -1, -1)
	end
  else
	cwd = ""
  end
